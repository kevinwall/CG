...
bool Triangle::intersect( const Ray& r, Surfel*& sf ) const
{
    ...
    Vector3<float> vertex0 = this->mesh->vertices[ this->v[0] ];
    Vector3<float> vertex1 = this->mesh->vertices[ this->v[1] ];  
    Vector3<float> vertex2 = this->mesh->vertices[ this->v[2] ];
    ...
    if (u < 0.0 || u > 1.0) // Acho que isso quer dizer que ele saiu do triângulo (estourou as coordenadas baricêntricas)
        return false;

    q = cross(s, edge1);
    v = f * dot(r.d, q);

    if (v < 0.0 || u + v > 1.0) // Mesma coisa do if acima
        return false;
    ...
    if (t > EPSILON) // ray intersection
    {
        ...
        sf->wo = -(r.d);
        sf->time = t;
        sf->n = unit_vector(cross(edge2,edge1));
        //cout << sf->n << "\n";
 // this->mesh->normals[*this->n]; // Talvez haja problema, visto que antes esperávamos uma normal apenas, e agora temos 3
...
bool Triangle::intersect_p( const Ray& r) const
{
    ...
    // Pode ser que aqui já possamos retornar o true.
...
create_triangle_mesh_shape( bool flip_normals, const ParamSet &ps )
{
	...
	TriangleMesh* mesh = new TriangleMesh(); // Default Ctro.

    // Retrieve filename.
    string filename = retrieve(ps, "filename", string{""}); // Retrieving data associated with 'filename' attrib.
    // Retrieve backface ON/OFF
    string bkf_on_str = retrieve(ps, "backface_cull", string{"unknown"}); 
    if ( bkf_on_str == "off" or bkf_on_str == "false" )
        bkfc_on = false;
    // Retrieve Reverse vertex order ON/OFF
    string rvo_str = retrieve(ps, "reverse_vertex_order", string{"unknown"});
    if ( rvo_str == "on" or rvo_str == "true" )
        reverse_vertex_order = true;
    // Retrieve compute normals flag
    string cn_str =  retrieve(ps, "compute_normals", string{"unknown"});
...
/*
        if (load_mesh_from_tag(ps,reverse_vertex_order, compute_normals, flip_normals, mesh ))
            std::cout << ">>> Mesh data successfuly loaded!\n";
        else
            std::cout << ">>> Mesh data loading failed!\n";
            */
        ...
        int ntriangles = retrieve(ps, "ntriangles", int{0});
        auto indices  = retrieve(ps, "indices", vector<int>{0} );
        auto vertices  = retrieve(ps, "vertices", vector<float>{0} );
        auto normals  = retrieve(ps, "normals", vector<float>{0} );
        auto uv  = retrieve(ps, "uv", vector<float>{0} );

        vector<Point3f> vert;
        for (int i = 0; i < vertices.size(); i+=3 ){
            vert.push_back(Point3f{vertices[i], vertices[i+1], vertices[i+2]});
        }

        vector<Normal3f> norm;
        for (int i = 0; i < normals.size(); i+=3 ){
            norm.push_back(Normal3f{normals[i], normals[i+1], normals[i+2]});
        }

        vector<Point2f> uvs;
        for (int i = 0; i < uv.size(); i+=2 ){
            uvs.push_back(Point2f{uv[i], uv[i+1]});
        }
...
bool load_mesh_data( const std::string & filename, bool rvo, bool cn, bool fn, TriangleMesh * md )
{
...
void extract_obj_data( const tinyobj::attrib_t& attrib,
                       ...
                       bool rvo, bool cn, bool fn, /* OUT */ TriangleMesh * md )
{
...
vector<Shape *>
create_triangle_mesh(TriangleMesh * mesh, bool backface_cull )
{
	...
	for ( int i = 0 ; i < mesh->n_triangles ; ++i )
		tris.push_back(new Triangle{mesh, i, backface_cull} );
